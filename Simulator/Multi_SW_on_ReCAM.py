...
def place_DB_seq_in_ReCAM(storage, start_row, first_row_col, last_row_col, shift_E_left_AD_col,
    ...
    # 2. set last_row bit (reset first_row_bit)
    last_row_vec = [0] * (len(seq_as_list) + 1)
    last_row_vec[0] = 1
    storage.loadData(last_row_vec, start_row, 1, last_row_col)
...
def Multi_SW_on_ReCAM(DB_sequences, query_seq):
    ...
    table_header_row = ["first_row_bit", "last_row_bit", "active_row_bit", "shift_E_left_AD_col",
    ...
    first_row_col = 0; last_row_col = 1; active_bit_col = 2; shift_E_left_AD_col = 3 #; stop_bit_col = 4
    ...
    max_AD_scores_col_index = num_of_bit_columns+7; temp_col_index = num_of_bit_columns+8
    ...
    for seq in DB_sequences:
        ...
        place_DB_seq_in_ReCAM(storage, copy_start_row, first_row_col, last_row_col, shift_E_left_AD_col,
    ...
    storage.loadData([1], copy_start_row, 1, last_row_col)
...
def test_Multi_SW_on_ReCAM():
    ...
    DB_seq[1] = "ACCG"
    DB_seq_list = []
    DB_seq_list.append(DB_seq[0])
    DB_seq_list.append(DB_seq[1])
    ...
    serial_result = Serial_SmithWaterman.main(input_seqA=DB_seq[0], input_seqB=query_seq)
    print("Serial result 1:", serial_result)

    serial_result = Serial_SmithWaterman.main(input_seqA=DB_seq[1], input_seqB=query_seq)
